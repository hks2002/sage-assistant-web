...
* @LastEditDate          : 2025-01-17 15:07:59
* @FilePath              : sage-assistant-web/src/components/echarts/EchartCustomerSummaryAmountByPNFamily.vue
...
function doUpdate() {
  ...
  axiosGet('/Data/CustomerSummaryAmountTopByPNFamily', {
    Site: props.site,
...
function prepareData() {
  ...
  let pie2Items = uniq(map(data, 'PNFamily'))
  let dataByPie2Items = groupBy(data, 'PNFamily')

  let pie2Data = []
  pie2Items.forEach((item) => {
    pie2Data.push({
      PNFamily: dataByPie2Items[item][0]['PNFamily'],
...
function setEchart() {
  ...
  const shortTitle = `${preTitle} ${t('S.SALES_ORDER_AMOUNT_BY_PN_FAMILY')}`
