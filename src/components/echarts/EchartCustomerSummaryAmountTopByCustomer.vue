...
* @LastEditDate          : 2025-01-17 15:09:13
* @FilePath              : sage-assistant-web/src/components/echarts/EchartCustomerSummaryAmountTopByCustomer.vue
...
import { forEach, groupBy, map, maxBy, sample, sortBy, sumBy, uniq } from 'lodash-es'
...
function prepareData() {
  ...
  forEach(groupBy(data, 'customerName'), (group) => {
    const val = sumBy(group, 'sumUSD')
    topTarget.push({ target: group.at(0)['customerName'], val: val })
  })
...
function setEchart() {
  const shortTitle = `${props.site} ${t('S.SALES_ORDER_AMOUNT_BY_CUSTOMER')} Top ${props.limit}`
  ...
  eChartOption = {
    ...
    grid: { left: '150', right: '30' },
    xAxis: {
      type: 'value',
      axisLabel: {
        formatter: '{value}\nUSD'
      },
      max: data.length > 0 ? maxBy(data, 'siteAllUSD')['siteAllUSD'] : 0
    },
    yAxis: {
      type: 'category',
      axisLabel: {
