...
* @LastEditDate          : 2025-01-17 15:07:35
* @FilePath              : sage-assistant-web/src/components/echarts/EchartCustomerSummaryAmountByCustomer.vue
...
function doUpdate() {
  ...
  axiosGet('/Data/CustomerSummaryAmountTopByCustomer', {
    Site: props.site,
...
function prepareData() {
  ...
  // for customer summary
  let pie2Items = uniq(map(data, 'customerCode'))
  let dataByPie2Items = groupBy(data, 'customerCode')

  let pie2Data = []
  pie2Items.forEach((item) => {
    pie2Data.push({
      customerCode: dataByPie2Items[item][0]['customerCode'],
      customerName: dataByPie2Items[item][0]['customerName'],
...
function setEchart() {
  ...
  const shortTitle = `${preTitle} ${t('S.SALES_ORDER_AMOUNT_BY_CUSTOMER')} `
  const longTitle = `${shortTitle} (${props.dateFrom}-->${props.dateTo})`

  // data is ready,set echart option
  eChartOption = {
    title: defaultTitle(shortTitle, ''),
