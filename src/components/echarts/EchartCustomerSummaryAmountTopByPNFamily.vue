...
* @LastEditDate          : 2025-01-17 15:09:34
* @FilePath              : sage-assistant-web/src/components/echarts/EchartCustomerSummaryAmountTopByPNFamily.vue
...
function prepareData() {
  ...
  forEach(groupBy(data, 'PNFamily'), (group) => {
    const val = sumBy(group, 'sumUSD')
    topTarget.push({ target: group.at(0)['PNFamily'], val: val })
  })
...
function setEchart() {
  const shortTitle = `${props.site} ${t('S.SALES_ORDER_AMOUNT_BY_PN_FAMILY')} Top ${props.limit}`
