...
* @CreatedDate           : 2024-09-18 18:28:00
* @LastEditors           : Robert Huang<56649783@qq.com>
* @LastEditDate          : 2025-01-19 23:45:47
* @FilePath              : sage-assistant-web/src/components/echarts/EchartTodoLongTimeNC.vue
...
function doUpdate() {
  ...
  axiosGet('/Data/LongTimeNC?Site=' + props.site)
    .then((response) => {
      data = response
      prepareData()
      setEchart()
...
function prepareData() {
  ...
  data.forEach((row) => {
    row['days'] = date.getDateDiff(row['firstNCDate'], newDate, 'days')
  })
...
function setEchart() {
  const title = t('S.LONGTIME_NC') + ` [${props.site}]`
